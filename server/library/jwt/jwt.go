...
// AuthJwt provides a Json-Web-Token authentication implementation. On failure, a 401 HTTP response
...
type AuthJwt struct {
	...
	// User can define own Invalid RefreshToken func.
	RefreshTokenError func(*ghttp.Request, int, string)
	// User can define own LoginResponse func.
	LoginResponse func(*ghttp.Request, int, string, time.Time, g.Map)
...
var (
	...
	ErrMissingAuthenticatorFunc = errors.New("AuthJwt.Authenticator func is undefined")
...
// New for check error with AuthJwt
func New(m *AuthJwt) (*AuthJwt, error) {
...
func (mw *AuthJwt) readKeys() error {
	err := mw.privateKey()
	if err != nil {
		return err
	}
...
func (mw *AuthJwt) privateKey() error {
...
func (mw *AuthJwt) publicKey() error {
...
func (mw *AuthJwt) usingPublicKeyAlgo() bool {
...
func (mw *AuthJwt) MiddlewareInit() error {

	if mw.TokenLookup == "" {
		mw.TokenLookup = "header:Authorization"
	}
	if mw.RefreshTokenLookup == "" {
		mw.RefreshTokenLookup = "param:refreshToken"
	}
	...
	if mw.RefreshTimeout == 0 {
		mw.RefreshTimeout = time.Hour * time.Duration(168) //默认一周
	}
	if mw.TimeFunc == nil {
	...
	if mw.LoginResponse == nil {
		mw.LoginResponse = func(r *ghttp.Request, code int, token string, expire time.Time, claims g.Map) {
			r.Response.WriteJson(g.Map{
				"code":   http.StatusOK,
				"token":  token,
...
// MiddlewareFunc makes AuthJwt implement the Middleware interface.
func (mw *AuthJwt) MiddlewareFunc() ghttp.HandlerFunc {
	return func(r *ghttp.Request) {
		mw.middlewareImpl(r)
	}
}

func (mw *AuthJwt) middlewareImpl(r *ghttp.Request) {
...
func (mw *AuthJwt) GetClaimsFromJWT(r *ghttp.Request) (MapClaims, error) {
...
func (mw *AuthJwt) LoginHandler(r *ghttp.Request) {
	...
	if err != nil {
		mw.unauthorized(r, http.StatusOK, mw.HTTPStatusMessageFunc(err, r))
		return
	}
	...
	mw.LoginResponse(r, http.StatusOK, tokenString, expire, claims)
}

func (mw *AuthJwt) signedString(token *jwt.Token) (string, error) {
...
func (mw *AuthJwt) RefreshHandler(r *ghttp.Request) {
	tokenString, expire, err := mw.RefreshToken(r)
	if err != nil {
		mw.invalidRefreshToken(r, http.StatusUnauthorized, mw.HTTPStatusMessageFunc(err, r))
		return
	}
...
func (mw *AuthJwt) RefreshToken(r *ghttp.Request) (string, time.Time, error) {
	claims, err := mw.CheckIfRefreshTokenExpire(r)
	if err != nil {
		return "", time.Now(), err
	}
...
func (mw *AuthJwt) CheckIfTokenExpire(r *ghttp.Request) (jwt.MapClaims, error) {
	token, err := mw.ParseToken(r)
...
func (mw *AuthJwt) CheckIfRefreshTokenExpire(r *ghttp.Request) (jwt.MapClaims, error) {
	token, err := mw.ParseRefreshToken(r)
	...
	if origIat < mw.TimeFunc().Add(-mw.RefreshTimeout).Unix() {
		return nil, errors.New("刷新TOKEN已经过期了")
	}

	isRefresh, ok := claims["is_refresh"]
	if !ok || gconv.String(isRefresh) != "1" {
...
func (mw *AuthJwt) TokenGenerator(data interface{}) (string, time.Time, error) {
	token := jwt.New(jwt.GetSigningMethod(mw.SigningAlgorithm))
	claims := token.Claims.(jwt.MapClaims)
...
// 生成刷新TOKEN
func (mw *AuthJwt) RefreshTokenGenerator(data interface{}) (string, time.Time, error) {
	token := jwt.New(jwt.GetSigningMethod(mw.SigningAlgorithm))
	claims := token.Claims.(jwt.MapClaims)
...
func (mw *AuthJwt) ParseRefreshToken(r *ghttp.Request) (*jwt.Token, error) {
	var token string
	var err error

	methods := strings.Split(mw.RefreshTokenLookup, ",")
	...
	return jwt.Parse(token, func(t *jwt.Token) (interface{}, error) {
		...
		r.SetParam("JWT_REFRESH_TOKEN", token)

		return mw.Key, nil
	})
}
func (mw *AuthJwt) jwtFromHeader(r *ghttp.Request, key string) (string, error) {
...
func (mw *AuthJwt) jwtFromQuery(r *ghttp.Request, key string) (string, error) {
...
func (mw *AuthJwt) jwtFromCookie(r *ghttp.Request, key string) (string, error) {
...
func (mw *AuthJwt) jwtFromParam(r *ghttp.Request, key string) (string, error) {
	token := r.GetString(key)
	if token == "" {
...
func (mw *AuthJwt) ParseToken(r *ghttp.Request) (*jwt.Token, error) {
	var token string
	var err error
...
func (mw *AuthJwt) unauthorized(r *ghttp.Request, code int, message string) {
...
func (mw *AuthJwt) invalidRefreshToken(r *ghttp.Request, code int, message string) {
	r.Header.Set("WWW-Authenticate", "JWT realm="+mw.Realm)
	mw.RefreshTokenError(r, code, message)
	if !mw.DisabledAbort {
		r.ExitAll()
	}

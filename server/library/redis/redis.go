...
var NilError = errors.New(NoResult)
...
// ZRangeByScoreAndOffsetCount 返回有序集 key 中，所有 score 值介于 min 和 max 之间(包括等于 min 或 max )的成员。有序集成员按 score 值递增(从小到大)次序排列 带分页
...
func (r *Client) TTL(key string) (int64, error) {
	val, err := redis.Int64(g.Redis().Do("TTL", key))
	return val, err
}

func (r *Client) GetSet(key string, value interface{}) (interface{}, error) {
	return g.Redis().Do("GETSET", key, value)
}

func (r *Client) SetNx(key string, value interface{}) (interface{}, error) {
	return g.Redis().Do("SETNX", key, value)
}
